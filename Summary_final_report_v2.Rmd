...
### So did soil testing lead to higher recommended P rates in 2019?
...
ggplot(P_zone_2019, aes(x=p_rec)) +
...
ggplot(P_zone_2020, aes(x=p_rec)) +
  geom_histogram(aes(y = stat(count) / sum(count))) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1))+
  #facet_wrap(.~ rainfall_class_f)+
  theme_bw() +
...
```{r soil testing recom plot 2019 P, echo=FALSE, message=TRUE, warning=FALSE}
### Duplication may have crept in ?? I only want one zoneID strip type and rate
...
# Count of P zone in 2019
```{r Count of P zone in 2019, echo=FALSE, message=TRUE, warning=FALSE}
P_zone_count_2019
```

# Count of P zone in 2019 that had rec rate  >= 5
zones with a likley resposne to P fert
```{r Count of P zone in 2019 that had rec rate more than 5, echo=FALSE, message=TRUE, warning=FALSE}
table1_2019_P
```

# Count of P zone in 2019 rec rate suggested changing planned practice

```{r Count of P zone in 2019 rec rate change practice, echo=FALSE, message=TRUE, warning=FALSE}
table2_2019_P
```


```{r soil testing recom plot 2019 N, echo=FALSE, message=TRUE, warning=FALSE}
...
# Count of N zone in 2019
```{r Count of N zone in 2019, echo=FALSE, message=TRUE, warning=FALSE}
N_zone_count_2019
```

# Count of N zone in 2019 that had rec rate  > 0
zones with a likley resposne to P fert
```{r Count of N zone in 2019 that had rec rate more than 0, echo=FALSE, message=TRUE, warning=FALSE}
table1_2019_N
```

# Count of N zone in 2019 rec rate suggested changing planned practice

```{r Count of N zone in 2019 rec rate change practice, echo=FALSE, message=TRUE, warning=FALSE}
table2_2019_N
```



```{r soil testing recom plot 2020 P and N, message=TRUE, warning=FALSE, include=FALSE}
### Duplication may have crept in ?? I only want one zoneID strip type and rate
...
# Count of P zone in 2020
```{r Count of P zone in 2020, echo=FALSE, message=TRUE, warning=FALSE}
P_zone_count2020
```

# Count of P zone in 2020 that had rec rate  > = 5
zones with a likley resposne to P fert
```{r Count of P zone in 2020 that had rec rate more than 0, echo=FALSE, message=TRUE, warning=FALSE}
table1_2020_P
```

# Count of P zone in 2020 rec rate suggested changing planned practice

```{r Count of P zone in 2020 rec rate change practice, echo=FALSE, message=TRUE, warning=FALSE}
table2_2020_P
```

# Count of N zone in 2020
```{r Count of N zone in 2020, echo=FALSE, message=TRUE, warning=FALSE}
N_zone_count_2020
```

# Count of N zone in 2020 that had rec rate  >  0
zones with a likley resposne to P fert
```{r Count of N zone in 2020 that had rec rate more than 0, echo=FALSE, message=TRUE, warning=FALSE}
table1_2020_N
```

# Count of N zone in 2020 rec rate suggested changing planned practice

```{r Count of N zone in 2020 rec rate change practice, echo=FALSE, message=TRUE, warning=FALSE}
table2_2020_N
```

## Did the soil test recommendations lead to greater gross margins?

We don't always have a match with the recom rates and what as applied.
So we have calualted the best match based on:
how much N / P was applied in the season (or we were told about)
what the recommeded rate was calualted from the soil test.
