...
t.test_details_rec_rates_20 <- t.test_details_rec_rates_20 %>% 
  dplyr::mutate(
    variable_costs = case_when(
      ...
      Strip_Type == "N Strip" & rainfall_class == "medium" ~  348,
...
N
When soil test indicates that a yield respose is likely the threshold of Rec_N_jax >0
...
### 2019 N rates GSP & Rec rates yield results
```{r 2019 Yld N plots GSP vs approx all data, echo=FALSE, message=TRUE, warning=FALSE}
N_2019_GSP <- All_zone_rates_2019 %>%
  filter(Strip_Type == "N Strip") %>% 
  filter(GSP == "GSP") %>% 
  #filter(rec_rate_approx_n == "approx_rec_rate") %>%
...
#View(N_2019_GSP)
N_2019_GSP %>% group_by(rainfall_class)%>%
  summarise(count = n())
...
## what is the approx recom rate? what is the approx GSP
N_2019_approx_rate %>% group_by(rainfall_class)%>%
  summarise(count = n())
...
N_2019_approx_rate_GSP_plot_long_YLD <- N_2019_approx_rate_GSP %>% 
  dplyr::select (Zone_ID,rainfall_class, Yld_rec_rate,Yld_GSP ) 



N_2019_approx_rate_GSP_plot_long_YLD <- N_2019_approx_rate_GSP_plot_long_YLD %>% 
...
N_2019_YLD_clm <-N_2019_approx_rate_GSP_plot_long_YLD %>%  
  ggplot(aes(x = rainfall_class, y = Yld, fill = strip)) +
  geom_bar(aes(rainfall_class, Yld, fill = as.factor(strip)),
           position = "dodge", stat = "summary", fun.y = "median")+
  ylim(0, 10)+
  theme_bw()+
  scale_fill_manual(values=c("darkblue", "grey30"),
                    name="strip",
                    breaks=c("Yld_rec_rate", "Yld_GSP"),
...
N_2019_YLD_box <- N_2019_approx_rate_GSP_plot_long_YLD %>%  
  ggplot(aes(x = rainfall_class, y = Yld, color =strip)) +
  geom_point(position = position_dodge(width=0.75))+
  scale_color_manual(values=c("grey30", "darkblue"))+   
...
N_2019_YLD_box
 ...
 ggsave(N_2019_YLD_clm,
        filename = 
          "W:/value_soil_testing_prj/Yield_data/scripts/final_report/soil_testing_final_report/N_2019_YLD_clm.png", 
        device = "png" ,
        dpi=600,
        width = 16,
        height = 9)
 
 ggsave(N_2019_YLD_box,
        filename = 
          "W:/value_soil_testing_prj/Yield_data/scripts/final_report/soil_testing_final_report/N_2019_YLD_box.png", 
        device = "png" ,
        dpi=600,
        width = 16,
        height = 9)
...
```
### 2020 N rates GSP & Rec rates yield results

```{r 2020 Yld N plots GSP vs approx all data, echo=FALSE, message=TRUE, warning=FALSE}
N_2020_GSP <- All_zone_rates_2020 %>%
  filter(Strip_Type == "N Strip") %>% 
  filter(GSP == "GSP") %>% 
  #filter(rec_rate_approx_n == "approx_rec_rate") %>%
...
#View(N_2020_GSP)
N_2020_GSP %>% group_by(rainfall_class)%>%
  summarise(count = n())
...
## what is the approx recom rate? what is the approx GSP
N_2020_approx_rate %>% group_by(rainfall_class)%>%
  summarise(count = n())
...
## need to make this into a long dataset
N_2020_approx_rate_GSP_plot_long_YLD <- N_2020_approx_rate_GSP %>% 
  dplyr::select (Zone_ID,rainfall_class, Yld_rec_rate,Yld_GSP ) 



N_2020_approx_rate_GSP_plot_long_YLD <- N_2020_approx_rate_GSP_plot_long_YLD %>% 
  pivot_longer(cols = starts_with("Yld"),
               names_to = "strip",
               values_to = "Yld")
...
N_2020_YLD_clm <-N_2020_approx_rate_GSP_plot_long_YLD %>%  
  ggplot(aes(x = rainfall_class, y = Yld, fill = strip)) +
  geom_bar(aes(rainfall_class, Yld, fill = as.factor(strip)),
           position = "dodge", stat = "summary", fun.y = "median")+
  ylim(0, 10)+
...
N_2020_YLD_box <- N_2020_approx_rate_GSP_plot_long_YLD %>%  
  ggplot(aes(x = rainfall_class, y = Yld, color =strip)) +
  geom_point(position = position_dodge(width=0.75))+
  scale_color_manual(values=c("grey30", "darkblue"))+   
  geom_boxplot(alpha = 0.1, width=0.75,aes(fill = strip)) +
...
ggsave(N_2020_YLD_clm,
       filename = 
         "W:/value_soil_testing_prj/Yield_data/scripts/final_report/soil_testing_final_report/N_2020_YLD_clm.png", 
       device = "png" ,
       dpi=600,
       width = 16,
       height = 9)

ggsave(N_2020_YLD_box,
       filename = 
         "W:/value_soil_testing_prj/Yield_data/scripts/final_report/soil_testing_final_report/N_2020_YLD_box.png", 
       device = "png" ,
       dpi=600,
       width = 16,
       height = 9)
...
```{r 2019 Yld P plots GSP vs approx all data, echo=FALSE, message=TRUE, warning=FALSE}
P_2019_GSP <- All_zone_rates_2019 %>%
  filter(Strip_Type == "P Strip") %>% 
  filter(GSP == "GSP") %>% 
  #filter(rec_rate_approx_n == "approx_rec_rate") %>%
...
#View(N_2019_GSP)
P_2019_GSP %>% group_by(rainfall_class)%>%
  summarise(count = n())
...
## what is the approx recom rate? what is the approx GSP
P_2019_approx_rate %>% group_by(rainfall_class)%>%
  summarise(count = n())
...
P_2019_approx_rate_GSP %>% group_by(rainfall_class)%>%
  summarise(count = n())
str(P_2019_approx_rate_GSP)

## need to make this into a long dataset
P_2019_approx_rate_GSP_plot_long_YLD <- P_2019_approx_rate_GSP %>% 
  dplyr::select (Zone_ID,rainfall_class, Yld_rec_rate,Yld_GSP ) 



P_2019_approx_rate_GSP_plot_long_YLD <- P_2019_approx_rate_GSP_plot_long_YLD %>% 
  pivot_longer(cols = starts_with("Yld"),
               names_to = "strip",
               values_to = "Yld")
...
P_2019_YLD_clm <-P_2019_approx_rate_GSP_plot_long_YLD %>%  
  ggplot(aes(x = rainfall_class, y = Yld, fill = strip)) +
  geom_bar(aes(rainfall_class, Yld, fill = as.factor(strip)),
           position = "dodge", stat = "summary", fun.y = "median")+
  ylim(0, 10)+
...
P_2019_YLD_box <- P_2019_approx_rate_GSP_plot_long_YLD %>%  
  ggplot(aes(x = rainfall_class, y = Yld, color =strip)) +
  geom_point(position = position_dodge(width=0.75))+
  scale_color_manual(values=c("grey30", "darkblue"))+   
  geom_boxplot(alpha = 0.1, width=0.75,aes(fill = strip)) +
...
ggsave(P_2019_YLD_clm,
       filename = 
         "W:/value_soil_testing_prj/Yield_data/scripts/final_report/soil_testing_final_report/P_2019_YLD_clm.png", 
       device = "png" ,
       dpi=600,
       width = 16,
       height = 9)

ggsave(P_2019_YLD_box,
       filename = 
         "W:/value_soil_testing_prj/Yield_data/scripts/final_report/soil_testing_final_report/P_2019_YLD_box.png", 
       device = "png" ,
       dpi=600,
       width = 16,
       height = 9)
...
```{r 2020 Yld P plots GSP vs approx all data, echo=FALSE, message=TRUE, warning=FALSE}
P_2020_GSP <- All_zone_rates_2020 %>%
  filter(Strip_Type == "P Strip") %>% 
  filter(GSP == "GSP") %>% 
  #filter(rec_rate_approx_n == "approx_rec_rate") %>%
...
#View(N_2020_GSP)
P_2020_GSP %>% group_by(rainfall_class)%>%
  summarise(count = n())
...
## what is the approx recom rate? what is the approx GSP
P_2020_approx_rate %>% group_by(rainfall_class)%>%
  summarise(count = n())
...
## need to make this into a long dataset
P_2020_approx_rate_GSP_plot_long_YLD <- P_2020_approx_rate_GSP %>% 
  dplyr::select (Zone_ID,rainfall_class, Yld_rec_rate,Yld_GSP ) 



P_2020_approx_rate_GSP_plot_long_YLD <- P_2020_approx_rate_GSP_plot_long_YLD %>% 
  pivot_longer(cols = starts_with("Yld"),
               names_to = "strip",
               values_to = "Yld")
...
P_2020_YLD_clm <-P_2020_approx_rate_GSP_plot_long_YLD %>%  
  ggplot(aes(x = rainfall_class, y = Yld, fill = strip)) +
  geom_bar(aes(rainfall_class, Yld, fill = as.factor(strip)),
           position = "dodge", stat = "summary", fun.y = "median")+
  ylim(0, 10)+
...
P_2020_YLD_box <- P_2020_approx_rate_GSP_plot_long_YLD %>%  
  ggplot(aes(x = rainfall_class, y = Yld, color =strip)) +
  geom_point(position = position_dodge(width=0.75))+
  scale_color_manual(values=c("grey30", "darkblue"))+   
  geom_boxplot(alpha = 0.1, width=0.75,aes(fill = strip)) +
...
ggsave(P_2020_YLD_clm,
       filename = 
         "W:/value_soil_testing_prj/Yield_data/scripts/final_report/soil_testing_final_report/P_2020_YLD_clm.png", 
       device = "png" ,
       dpi=600,
       width = 16,
       height = 9)

ggsave(P_2020_YLD_box,
       filename = 
         "W:/value_soil_testing_prj/Yield_data/scripts/final_report/soil_testing_final_report/P_2020_YLD_box.png", 
       device = "png" ,
       dpi=600,
       width = 16,
       height = 9)
...
# yld plots no rainfall data
 
```{r Yld P N plots GSP vs approx all data, echo=FALSE, message=TRUE, warning=FALSE}
# View(P_2020_approx_rate_GSP_plot_long_YLD)
# View(P_2019_approx_rate_GSP_plot_long_YLD)
# 
# View(N_2020_approx_rate_GSP_plot_long_YLD)
# View(N_2019_approx_rate_GSP_plot_long_YLD)
...
N_P_2019_20_approx_rate_GSP_plot_long_YLD <- bind_rows(P_2020_approx_rate_GSP_plot_long_YLD,
                                                       P_2019_approx_rate_GSP_plot_long_YLD,
                                                       N_2020_approx_rate_GSP_plot_long_YLD,
                                                       N_2019_approx_rate_GSP_plot_long_YLD)

# names(N_P_2019_20_approx_rate_GSP_plot_long_YLD)
# str(N_P_2019_20_approx_rate_GSP_plot_long_YLD)


levels(N_P_2019_20_approx_rate_GSP_plot_long_YLD$strip) <- list(Yld_rec_rate  = "Approx. rec rate", Yld_GSP = "GSP")

N_P_2019_20_approx_rate_GSP_plot_long_YLD <- N_P_2019_20_approx_rate_GSP_plot_long_YLD %>% 
  dplyr::mutate(
    label_graph = case_when(
    strip == "Yld_rec_rate" ~ "Approx. rec rate",
    strip == "Yld_GSP" ~ "GSP"
  ))

N_P_2019_20_approx_rate_GSP_plot_long_YLD$label_graph <- factor(N_P_2019_20_approx_rate_GSP_plot_long_YLD$label_graph, 
                                            levels = c("GSP", "Approx. rec rate"))

PN_2019_20_YLD_box <- N_P_2019_20_approx_rate_GSP_plot_long_YLD %>%  
  ggplot(aes(x = label_graph, y = Yld)) +
  geom_point(position = position_dodge(width=0.75))+
  theme_bw()+
  scale_color_manual(values=c("grey30", "darkblue"))+   
  geom_boxplot(alpha = 0.1, width=0.75,aes(fill = strip)) +
...
ggsave(PN_2019_20_YLD_box,
       filename = 
         "W:/value_soil_testing_prj/Yield_data/scripts/final_report/soil_testing_final_report/PN_2019_20_YLD_box.png", 
       device = "png" ,
       dpi=600,
       width = 16,
       height = 9)
...
# N_2019_GM_clm <-N_2019_approx_rate_GSP_plot_long_GM %>%  
...
############################################################################################################
N_2019_approx_rate_GSP_plot_long_GM_summary <- N_2019_approx_rate_GSP_plot_long_GM %>% 
  group_by(rainfall_class,strip ) %>% 
  summarise(GM_median = median(GM, na.rm= FALSE))


N_2019_GM_clm <-N_2019_approx_rate_GSP_plot_long_GM_summary %>%
  ggplot()+
  geom_col( aes(x=rainfall_class, y=GM_median, fill = as.factor(strip)), position = "dodge")+
  ylim(0,1800)+
  theme_bw()+
  scale_fill_manual(values=c("darkblue", "grey30"),
                    name="strip",
...
N_2019_GM_box <- N_2019_approx_rate_GSP_plot_long_GM %>%  
...
ggsave(N_2019_GM_clm,
       filename = 
         "W:/value_soil_testing_prj/Yield_data/scripts/final_report/soil_testing_final_report/N_2019_GM_clm.png", 
       device = "png" ,
       dpi=600,
       width = 16,
       height = 9)

ggsave(N_2019_GM_box,
       filename = 
         "W:/value_soil_testing_prj/Yield_data/scripts/final_report/soil_testing_final_report/N_2019_GM_box.png", 
       device = "png" ,
       dpi=600,
       width = 16,
       height = 9)
...
# P_2019_GM_clm <-P_2019_approx_rate_GSP_plot_long_GM %>%  
#   ggplot(aes(x = rainfall_class, y = GM, fill = strip)) +
#   geom_bar(aes(rainfall_class, GM, fill = as.factor(strip)),
#            position = "dodge", stat = "summary", fun.y = "median")+
#    ylim(0,1500)+
...
############################################################################################################
P_2019_approx_rate_GSP_plot_long_GM_summary <- P_2019_approx_rate_GSP_plot_long_GM %>% 
  group_by(rainfall_class,strip ) %>% 
  summarise(GM_median = median(GM, na.rm= FALSE))


P_2019_GM_clm <-P_2019_approx_rate_GSP_plot_long_GM_summary %>%
  ggplot()+
  geom_col( aes(x=rainfall_class, y=GM_median, fill = as.factor(strip)), position = "dodge")+
  ylim(0,1800)+
  theme_bw()+
...
P_2019_GM_box <- P_2019_approx_rate_GSP_plot_long_GM %>%  
  ggplot(aes(x = rainfall_class, y = GM, color =strip)) +
  geom_point(position = position_dodge(width=0.75))+
  scale_color_manual(values=c("grey30", "darkblue"))+   
  geom_boxplot(alpha = 0.1, width=0.75,aes(fill = strip)) +
...
ggsave(P_2019_GM_clm,
       filename = 
         "W:/value_soil_testing_prj/Yield_data/scripts/final_report/soil_testing_final_report/P_2019_GM_clm.png", 
       device = "png" ,
       dpi=600,
       width = 16,
       height = 9)

ggsave(P_2019_GM_box,
       filename = 
         "W:/value_soil_testing_prj/Yield_data/scripts/final_report/soil_testing_final_report/P_2019_GM_box.png", 
       device = "png" ,
       dpi=600,
       width = 16,
       height = 9)
...
# N_2020_GM_clm <-N_2020_approx_rate_GSP_plot_long_GM %>%  
#   ggplot(aes(x = rainfall_class, y = GM, fill = strip)) +
#   geom_bar(aes(rainfall_class, GM, fill = as.factor(strip)),
#            position = "dodge", stat = "summary", fun.y = "median")+
#    ylim(0,1500)+
...
############################################################################################################
N_2020_approx_rate_GSP_plot_long_GM_summary <- N_2020_approx_rate_GSP_plot_long_GM %>% 
  group_by(rainfall_class,strip ) %>% 
  summarise(GM_median = median(GM, na.rm= FALSE))


N_2020_GM_clm <-N_2020_approx_rate_GSP_plot_long_GM_summary %>%
  ggplot()+
  geom_col( aes(x=rainfall_class, y=GM_median, fill = as.factor(strip)), position = "dodge")+
  ylim(0,1800)+
  theme_bw()+
...
N_2020_GM_box <- N_2020_approx_rate_GSP_plot_long_GM %>%  
  ggplot(aes(x = rainfall_class, y = GM, color =strip)) +
  geom_point(position = position_dodge(width=0.75))+
  scale_color_manual(values=c("grey30", "darkblue"))+   
  geom_boxplot(alpha = 0.1, width=0.75,aes(fill = strip)) +
...
ggsave(N_2020_GM_clm,
       filename = 
         "W:/value_soil_testing_prj/Yield_data/scripts/final_report/soil_testing_final_report/N_2020_GM_clm.png", 
       device = "png" ,
       dpi=600,
       width = 16,
       height = 9)

ggsave(N_2020_GM_box,
       filename = 
         "W:/value_soil_testing_prj/Yield_data/scripts/final_report/soil_testing_final_report/N_2020_GM_box.png", 
       device = "png" ,
       dpi=600,
       width = 16,
       height = 9)
...
# P_2020_GM_clm <-P_2020_approx_rate_GSP_plot_long_GM %>%  
#   ggplot(aes(x = rainfall_class, y = GM, fill = strip)) +
#   geom_bar(aes(rainfall_class, GM, fill = as.factor(strip)),
#            position = "dodge", stat = "summary", fun.y = "median")+
#    ylim(0,1500)+
...
############################################################################################################
P_2020_approx_rate_GSP_plot_long_GM_summary <- P_2020_approx_rate_GSP_plot_long_GM %>% 
  group_by(rainfall_class,strip ) %>% 
  summarise(GM_median = median(GM, na.rm= FALSE))


P_2020_GM_clm <-P_2020_approx_rate_GSP_plot_long_GM_summary %>%
  ggplot()+
  geom_col( aes(x=rainfall_class, y=GM_median, fill = as.factor(strip)), position = "dodge")+
  ylim(0,1800)+
  theme_bw()+
...
P_2020_GM_box <- P_2020_approx_rate_GSP_plot_long_GM %>%  
  ggplot(aes(x = rainfall_class, y = GM, color =strip)) +
  geom_point(position = position_dodge(width=0.75))+
  scale_color_manual(values=c("grey30", "darkblue"))+   
  geom_boxplot(alpha = 0.1, width=0.75,aes(fill = strip)) +
...
ggsave(P_2020_GM_clm,
       filename = 
         "W:/value_soil_testing_prj/Yield_data/scripts/final_report/soil_testing_final_report/P_2020_GM_clm.png", 
       device = "png" ,
       dpi=600,
       width = 16,
       height = 9)

ggsave(P_2020_GM_box,
       filename = 
         "W:/value_soil_testing_prj/Yield_data/scripts/final_report/soil_testing_final_report/P_2020_GM_box.png", 
       device = "png" ,
       dpi=600,
       width = 16,
       height = 9)
...
## N and P 2019 and 2020 GM without rainfall data
```{r 2020 2019 GM  plots GSP vs approx all data, echo=FALSE, message=TRUE, warning=FALSE}
...
N_P_2019_20_approx_rate_GSP_plot_long_GM <- rbind(N_2019_approx_rate_GSP_plot_long_GM,
                                                  N_2020_approx_rate_GSP_plot_long_GM,
...
#str(N_P_2019_20_approx_rate_GSP_plot_long_GM)

levels(N_P_2019_20_approx_rate_GSP_plot_long_GM$strip) <- list(GM_rec_rate  = "Approx. rec rate", GM_GSP = "GSP")

N_P_2019_20_approx_rate_GSP_plot_long_GM <- N_P_2019_20_approx_rate_GSP_plot_long_GM %>% 
  dplyr::mutate(
    label_graph = case_when(
      strip == "GM_rec_rate" ~ "Approx. rec rate",
      strip == "GM_GSP" ~ "GSP"
    ))

N_P_2019_20_approx_rate_GSP_plot_long_GM$label_graph <- factor(N_P_2019_20_approx_rate_GSP_plot_long_GM$label_graph, 
                                                                levels = c("GSP", "Approx. rec rate"))

#str(N_P_2019_20_approx_rate_GSP_plot_long_GM)

summary_GM <- N_P_2019_20_approx_rate_GSP_plot_long_GM %>%  group_by(Strip_type, year,  strip, rainfall_class) %>% 
  summarise(GM_av = mean(GM , na.rm = TRUE ),
            st_dev = sd(GM , na.rm = TRUE),
            GM_median = median(GM , na.rm = TRUE ))

#View(summary_GM)

N_P_2019_20_approx_rate_GSP_plot_long_GM %>%  group_by(Strip_type, year,  strip) %>% 
  summarise(GM_av = mean(GM , na.rm = TRUE ),
            st_dev = sd(GM , na.rm = TRUE),
            GM_median = median(GM , na.rm = TRUE ))
...
PN_2019_20_GM_box <- N_P_2019_20_approx_rate_GSP_plot_long_GM %>%  
  ggplot(aes(x = label_graph, y = GM)) +
  geom_point(position = position_dodge(width=0.75))+
  theme_bw()+
  scale_color_manual(values=c("grey30", "darkblue"))+   
  geom_boxplot(alpha = 0.1, width=0.75,aes(fill = strip)) +
...
ggsave(PN_2019_20_GM_box,
       filename = 
         "W:/value_soil_testing_prj/Yield_data/scripts/final_report/soil_testing_final_report/PN_2019_20_GM_box.png", 
       device = "png" ,
       dpi=600,
       width = 16,
       height = 9)
...
"W:\value_soil_testing_prj\Yield_data\2020\processing\r_scripts\site_t_test\MSF\Andreson_Gleeson_2021 WS.R"

```{r 2020 case study 3, echo=FALSE, message=TRUE, warning=FALSE}

case_study3 <- All_zone_rates_2020 %>% 
  filter(Paddock_ID == "52411") %>% 
  dplyr::select(Strip_Type,
                Zone_ID,
                Paddock_tested,
                Total_N,
...
case_study3_zone_details <- case_study3 %>% 
  distinct(Zone_ID, .keep_all = TRUE) %>% 
dplyr::select(Zone_ID, Strip_Type,Paddock_tested,Total_N, Rec_N_jax, rec_rate_approx_n)
#dplyr::select(Zone_ID, Colwell, DGT, p_rec, rec_rate_approx_p) 

case_study3_zone_details

case_study3_N <- case_study3 %>% 
  mutate(Yld = round(Yld, 1),
         GM = round(GM, 0) ,
        Total_sum_N_content = round(Total_sum_N_content, 0))%>% 
  dplyr::select(Zone_ID, Total_sum_N_content, Yld , GM , GSP, rec_rate_approx_n) 
...
### Case study 3 with an average across for the entire strip (not just zones)

```{r 2020 case study 3 av strip, echo=FALSE, message=TRUE, warning=FALSE}
case_study3 <- All_zone_rates_2020 %>% 
  filter(Paddock_ID == "52411") %>% 
  dplyr::select(Strip_Type,
                Zone_ID,
...
whole_strip_study3 <- read.csv("W:/value_soil_testing_prj/Yield_data/2020/FSG/MSF_Frontier_Farming/Anderson/Gleesons/output/Gleesons_Yld_SegID_Zone.csv")



whole_strip_study3$GSP <- as.character(whole_strip_study3$GSP)
names(whole_strip_study3)

whole_strip_study3 <- whole_strip_study3 %>% 
  dplyr::select(Yield, Paddock_ID:Zone) 
...
### group the data by rate
whole_strip_study3 <- whole_strip_study3 %>%  group_by(Rate) %>% 
  summarise(yld_var_strip = mean(Yield, na.rm = TRUE))
...
whole_strip_study3 <- left_join(whole_strip_study3,case_study3) %>% 
  distinct(Rate, .keep_all = TRUE) %>% 
  dplyr::select(Strip_Type:Total_sum_P_content ,Rate, Yld =yld_var_strip, rainfall_class)
...
### Case study 3 with an average across the zones

```{r 2020 case study 3 av zones, echo=FALSE, message=TRUE, warning=FALSE}
case_study3_N
case_study3_N %>%  group_by(Total_sum_N_content) %>% 
  summarise(av_yld_zone = mean(Yld, na.rm = TRUE),
            av_GM_zone = mean(GM, na.rm = TRUE))
...
```{r 2020 alt GSP working, message=TRUE, warning=FALSE, include=FALSE}
Alt_2020 <- read.csv("W:/value_soil_testing_prj/Yield_data/2020/processing/r_outputs/merged_comparision_output/GSP_AltGSP_t_test_merged_3c.csv")
names(Alt_2020)
Alt_2020 <- Alt_2020 %>% 
  dplyr::select(Zone_ID, Strip_Type, Alt.GSP,GSP,input_file,  Significant_practical )
Alt_2020 <- Alt_2020 %>% 
  filter(!is.na(GSP))
...
#####
table1<-Alt_2020 %>% group_by(Strip_Type)%>%
  summarise(count_strip_type_alt = n())

table2 <- Alt_2020 %>% group_by(Strip_Type, Significant_practical)%>%
  summarise(count_strip_sig_alt = n())
table1
table2

table <- left_join(table2,table1)
table <- table  %>% 
  filter(!is.na(Significant_practical))
table <- table %>% 
  dplyr::mutate(per_alt_diff = (count_strip_sig_alt/count_strip_type_alt)*100)
#table
...
table1_all<-Alt_2020 %>% 
  summarise(count_strip_type_alt = n())
table1_all
table2_all <- Alt_2020 %>% group_by( Significant_practical)%>%
  summarise(count_strip_sig_alt = n())
#table1_all
...
table2_all <- table2_all %>% 
  dplyr::mutate(per_alt_diff = (count_strip_sig_alt/116)*100)
table2_all

```

for all the 116 zones with alt gsp these were the results:
```{r 2020 alt GSP all strips, echo=FALSE, message=TRUE, warning=FALSE}

table2_all

```

for all the 116 zones with alt gsp these were the results grouped into strip type:
```{r 2020 alt GSP all strips results, echo=FALSE, message=TRUE, warning=FALSE}
...
How many zone were not included in the final analysis for 2020?

```{r 2020 excluded from analysis, message=TRUE, warning=FALSE, include=FALSE}

all_zones2020_ex <- st_read("W:/value_soil_testing_prj/Yield_data/2020/All_Zones_2020_wgs84.shp")

all_zones2020_ex <- all_zones2020_ex %>%
  dplyr::select(Zone_ID, join_field, 
                source_dir,
                Paddock_ID,
                Status,
                XLSExclude,
...
#2.turn polygons into points - centriod
all_zones2020_ex_centroid = st_centroid(all_zones2020_ex)



all_zones2020_ex_df <- as.data.frame(all_zones2020_ex_centroid)
...
2020 P included vs excluded in final analsyis

Percenatge of zone used in analysis 

```{r 2020 P excluded from analysis, echo=FALSE, message=TRUE, warning=FALSE}

P_zone_total_2020 <- all_zones2020_ex_df %>% 
  filter(Strip_Type == "P Strip") %>% 
  distinct(Zone_ID , .keep_all = TRUE)%>% 
count()

P_zone_used_2020 <-All_zone_rates_2020 %>% 
  filter(Strip_Type == "P Strip") %>% 
  distinct(Zone_ID , .keep_all = TRUE)%>%
count()
...
(P_zone_used_2020 / P_zone_total_2020) *100

```


2020 N included vs excluded in final analsyis

Percenatge of zone used in analysis 

```{r 2020 N excluded from analysis, echo=FALSE, message=TRUE, warning=FALSE}

N_zone_total_2020 <- all_zones2020_ex_df %>% 
  filter(Strip_Type == "N Strip") %>% 
  distinct(Zone_ID , .keep_all = TRUE)%>% 
count()

N_zone_used_2020 <-All_zone_rates_2020 %>% 
  filter(Strip_Type == "N Strip") %>% 
  distinct(Zone_ID , .keep_all = TRUE)%>%
count()
...
(N_zone_used_2020 / N_zone_total_2020) *100

```
Total of zone used in 2020 analysis
```{r 2020 P + N excluded from analysis, echo=FALSE, message=TRUE, warning=FALSE}
...
((N_zone_used_2020+P_zone_used_2020) / (N_zone_total_2020 + P_zone_total_2020)) *100

```

otal of zone NOT used in 2020 analysis
```{r 2020 P + N NOT from analysis, echo=FALSE, message=TRUE, warning=FALSE}
...
used <- ((((N_zone_used_2020+P_zone_used_2020) / (N_zone_total_2020 + P_zone_total_2020)) *100))
