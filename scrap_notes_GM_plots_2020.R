...
GM_t.test_details_rec_rates2020 <- read.csv("W:/value_soil_testing_prj/Yield_data/2020/processing/processing_files/for_econmics/GM_t.test_details_rec_rates2020.csv")
GM_t.test_details_rec_rates2020 <- GM_t.test_details_rec_rates2020 %>%  dplyr::select(-X)
names(GM_t.test_details_rec_rates2020)
...
min(GM_t.test_details_rec_rates2020$GM, na.rm = TRUE)
GM_t.test_details_rec_rates2020  <- GM_t.test_details_rec_rates2020 %>% 
  mutate(GM = case_when(
    GM < 0 ~ 0,
    TRUE ~ GM
...
P_zone <- GM_t.test_details_rec_rates2020_P %>% 
...
names(GM_t.test_details_rec_rates2020)
unique(GM_t.test_details_rec_rates2020$GSP)
unique(GM_t.test_details_rec_rates2020$Strip_Type)

GM_t.test_details_rec_rates2020$GSP <- as.character(GM_t.test_details_rec_rates2020$GSP)
GM_t.test_details_rec_rates2020$Strip_Type <- as.character(GM_t.test_details_rec_rates2020$Strip_Type)
str(GM_t.test_details_rec_rates2020)

GM_t.test_details_rec_rates2020 <- GM_t.test_details_rec_rates2020 %>% mutate(soil_test_says_P = case_when(
...
GM_t.test_details_rec_rates2020 <- GM_t.test_details_rec_rates2020 %>% mutate(soil_test_says_N = case_when(
...
P_zones <- GM_t.test_details_rec_rates2020 %>%  filter(Strip_Type == "P Strip") %>% 
  distinct(Zone_ID, .keep_all = TRUE) %>% 
  summarise(count = n())

GM_t.test_details_rec_rates2020 %>%  filter(Strip_Type == "P Strip") %>% 
  distinct(Zone_ID, .keep_all = TRUE) %>% 
  group_by(soil_test_says_P) %>% 
  summarise(count = (n() / P_zones)*100)

N_zones <- GM_t.test_details_rec_rates2020 %>%  filter(Strip_Type == "N Strip") %>% 
  distinct(Zone_ID, .keep_all = TRUE) %>% 
  summarise(count = n())

GM_t.test_details_rec_rates2020 %>%  filter(Strip_Type == "N Strip") %>% 
  distinct(Zone_ID, .keep_all = TRUE) %>% 
...
unique(GM_t.test_details_rec_rates2020$GSP_Rec_both_p)
GM_t.test_details_rec_rates2020$GSP_Rec_both_p <- as.character(GM_t.test_details_rec_rates2020$GSP_Rec_both_p)

P_approx_GSP_zone_strip <- GM_t.test_details_rec_rates2020 %>%  
...
#### N #####
N_approx_GSP_zone_strip <- GM_t.test_details_rec_rates2020 %>%  
